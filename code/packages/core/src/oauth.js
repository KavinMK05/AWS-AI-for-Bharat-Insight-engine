 ...
 */
function getTokenPaths(environment, platform) {
...
export class OAuthTokenManager {
    ...
    async loadTokens(platform) {
        ...
        try {
            const [accessTokenResult, refreshTokenResult] = await Promise.all([
                this.ssmClient.send(new GetParameterCommand({ Name: paths.accessToken, WithDecryption: true })),
                this.ssmClient.send(new GetParameterCommand({ Name: paths.refreshToken, WithDecryption: true })),
            ]);
            const tokenSet = {
     ...
     */
    async getValidAccessToken(platform) {
        ...
        try {
            ...
            throw new OAuthRefreshError(platform, `Token refresh failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async refreshAccessToken(platform) {
        ...
        const [clientIdResult, clientSecretResult] = await Promise.all([
            this.ssmClient.send(new GetParameterCommand({ Name: paths.clientId, WithDecryption: true })),
            this.ssmClient.send(new GetParameterCommand({ Name: paths.clientSecret, WithDecryption: true })),
        ]);
        ...
        // Send SNS alert
        await this.publishAlert(platform, `OAuth token refresh needed for ${platform}. Manual re-authorization required.`);
     ...
     */
    async writeTokensToSSM(platform, tokenSet) {
        const paths = getTokenPaths(this.environment, platform);
        try {
            await Promise.all([
     ...
     */
    async publishAlert(platform, message) {
        try {
