 ...
 */
async function getRequiredValue(envVar, ssmPath, ssmClient) {
 ...
 */
function getOptionalValue(envVar) {
...
export async function loadConfig() {
    ...
    const [personaFilesBucket, lambdaDeploymentsBucket] = await Promise.all([
        ...
        getRequiredValue('LAMBDA_DEPLOYMENTS_BUCKET', `${ssmPrefix}/lambda-deployments-bucket`, ssmClient),
    ]);
    const adminAlertsTopic = await getRequiredValue('ADMIN_ALERTS_TOPIC_ARN', `${ssmPrefix}/admin-alerts-topic-arn`, ssmClient);
    ...
    if (isNaN(relevanceThreshold) || relevanceThreshold < 0 || relevanceThreshold > 100) {
        throw new ConfigValidationError('RELEVANCE_THRESHOLD', `RELEVANCE_THRESHOLD must be a number between 0 and 100, got: ${rawThreshold}`);
    }
    const config = {
        awsRegion: region,
        environment,
