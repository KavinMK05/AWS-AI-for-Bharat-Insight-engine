...
export const personaFileSchema = z
    ...
    tone: z.enum(['formal', 'casual', 'technical', 'humorous'], {
        message: 'tone must be one of: "formal", "casual", "technical", "humorous"',
    }),
    expertise_topics: z
    ...
    digest_schedule: z.enum(['daily', 'twice-daily', 'weekly'], {
        message: 'digest_schedule must be one of: "daily", "twice-daily", "weekly"',
    }),
    monitoring_interval: z.enum(['hourly', 'every-6h', 'daily'], {
        message: 'monitoring_interval must be one of: "hourly", "every-6h", "daily"',
    }),
})
    .transform((raw) => ({
    tone: raw.tone,
 ...
 */
export async function loadPersona(source, path, options) {
    ...
    try {
        ...
        throw new Error(`Failed to parse persona file as JSON (source: ${source}, path: ${path})`);
    }
...
// Internal Helpers
// ============================================================================
async function readRawJson(source, path, options) {
    if (source === 'local') {
    ...
    if (!bucketName) {
        throw new Error('S3 bucket name is required: pass options.bucketName or set PERSONA_FILES_BUCKET env var');
    }
    const client = options?.s3Client ?? new S3Client({});
    const command = new GetObjectCommand({ Bucket: bucketName, Key: path });
    const response = await client.send(command);
    if (!response.Body) {
        throw new Error(`S3 object body is empty (bucket: ${bucketName}, key: ${path})`);
    }
